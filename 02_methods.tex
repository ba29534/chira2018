...
For a maximum resolution of $\Delta x = 0.1$~pc, the corresponding maximum resolved density is $8~\times 10^3$~cm$^{-3}$ for gas at a temperature of 10~K \citepalias[e.g.][Eq.~15]{IbanezMejia2017}. {\bf We define MCs as regions above a fixed number density threshold with fiducial value $n_{\mathrm cloud}$~=~100~cm$^{-3}$.  We chose this threshold as it roughly corresponds to the density when CO becomes detectable.}
...
%mm [restating the intro from Heyer & Brunt 04] The VSF estimates the occurrence of symmetric motions (e.g., rotation, collapse, outflows), as well as rare events in random turbulent flows (e.g., supernovae) in velocity patterns.
...
\end{equation}
%mm Note that the scaling exponent of that power-law relation, $\zeta$, depends on many parameters, such as the order of the VSF, as well as the properties and composition of the studied turbulent flow, such as its geometry, compressibility, or magnetization.
...
%mm However, the differences are mostly negligible on scales we focus on. 
%Since these decompositions are difficult to perform on observational data, we focus on the total VSF.

%mm [moved discussion ahead of definition of density-weighted SF]
...
This 
%mm results in the commonly known prediction 
     predicts
that the kinetic energy decays with $E_{\mathrm{kin}}(k) \propto k^{-\frac{5}{3}}$, with $k = \frac{2 \pi}{\ell}$ being the wavenumber of the turbulence mode.
%
...
%mm The first several orders of the VSF have a physical meaning. 
...
%mm
\textbf{We choose the smallest lag of the fitting range to be equal to \mm{ten zones [CHECK!]}, sufficiently large to ensure that our fit does not include the numerical dissipation range.
%
	For more details of the fitting procedure we refer to Appendix~\ref{appFitting}.
}

%mm [now covered in Appendix A] In order to reduce the computational effort, we divide the range of 3D lag distances, $|\vec{\ell}|$, into 40 equidistantly separated bins ranging from~0.1 to 30~pc.
...
%mm Furthermore, we 
We 
\textbf{follow observational practice and} reduce the computational effort of this study by generally focusing on clouds {\bf defined by a density threshold.}
...
As in the case without a density threshold it would be too computationally expensive to compute all lags to all zones, we randomly choose a set of 5\% of the total number of zones as reference points and only compute relative velocities
%mm [this is now in Appendix A.]  to these zones only. We emphasise that this does not mean that we only calculate the relative velocities between these zones.
%Rather this subsample of zones represent the starting vectors $\vec{x}$ to which the velocities of all other zones $\vec{x} + \vec{\ell}$ in the same cube are compared to.

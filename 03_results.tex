...
%mm distribution of turbulent power within molecular clouds.
    \textbf{velocity structure within and around MCs}.
...
one at small scales at \mm{[GIVE MINIMUM SCALE HERE]} $\ell \lesssim$~3~pc, a second one within 3~pc~$\lesssim \ell \lesssim$~10--15~pc, and the last one at large scales with $\ell >$~15~pc. \mm{[AND GIVE OUTER SCALE HERE]}
...
%mm Furthermore, this implies that we can only reliably examine turbulence within the simulations after 1.5~Myr and carefully need to take this into account in the further discussion 
...
The \textbf{later} examples represent the clouds 
%mm at later stages of their evolution]
when the VSFs are dominated by sources that drive the flow within the clouds in a more 
%mm extreme 
    \textbf{coherent}
way.
Fig.~\ref{pic:results:vsf_example}(b) shows the VSF of \texttt{M3} at a time when the cloud has just been hit by a SN %mm shock front.
    \textbf{blast wave}. 
...
%mm [removed -- wait for discussion in next section; otherwise requires too much forward referencing.]
%All this results in a steeper slope of the VSF 
%\textbf{compared to the same cloud prior to shock impact (see Fig.~\ref{pic:results:zeta_all} and discussion in Sect.~\ref{results:normal})}.
...
Gas moves into the inner regions of the cloud, reducing the average lag distances, %mm but not necessarily the relative velocities. The latter may even be accelerated by the infall.
     \textbf{while being accelerated by the infall to higher velocity.}
...
\textbf{
	Note that these 
%mm differences partly origin from the fact 
    flows mean 
that the turbulence we study here is not uniform or isotropic, as was assumed by \citet{She1994} and \citet{Boldyrev2002}.  Thus, derived VSFs are not only functions of time and lag, but also the direction of $\vec{\ell}$, as that direction defines at which scales of $\ell$ large scale structures become relevant.
...
\textbf{
	Note that the nearby SNe that we mark in Figs.~\ref{pic:results:zeta_all} and \ref{pic:results:z_all} and consider in the analysis were listed by \citetalias{IbanezMejia2017} as exploding
%mm This means that all SNe that are marked in Figs.~\ref{pic:results:zeta_all} and \ref{pic:results:z_all} explode 
at a radius of up to 100~pc from the clouds' centres of mass. The shock fronts move at average speeds of 50--100~km~s$^{-1}$ through the ISM, so it can take the blast waves more than 1~Myr to reach the clouds. Thus, it is important to keep in mind that the MCs do not react immediately to SNe, and that the time between the explosion of a SN and the interaction of its shock front with one of the clouds varies depending on the distance, as well as as the composition of the ISM along the propagation path.
}
...
%mm By doing so
     To do this, 
we derived the relative velocities based on the 3D velocity vectors from the simulations.
...
\textbf{However, there are also} cases in which the 1D VSF 
%mm evolves temporarily or completely 
     temporarily evolves completely
differently than the 3D VSF.
...
%mm
     \textbf{which focus attention on the dense regions of strongest collapse}. 
...
Other studies, have shown that a significantly higher refinement is needed to reliably resolve turbulent structures and flows within disks to resolve turbulence
%mm [added the Heidelberg citation]
     \citep{Federrath2011, Turk2012}.  
...
%mm [Much of the refinement happened prior to turning on self-gravity due to compression, isn't that right?]. This suggests that we can reliably trust the findings in this paper after the clouds have evolved for approximately 1.5~Myr 
\citep[see also][]{IbanezMejia2017,Seifried2017b}.

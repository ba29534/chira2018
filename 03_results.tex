...
\textbf{
	Note that we only consider "near-by" SNe in the analysis as they have been listed by  \citetalias{IbanezMejia2017}. 
	This means that all SNe that are marked in Figs.~\ref{pic:results:zeta_all} and \ref{pic:results:z_all} explode within a radius of up to 100~pc from the clouds' centres of mass. 
	Assuming that the shock fronts move at an average speeds of 50--100~km~s$^{-1}$ through the ISM it takes the blast waves around 1~Myr to reach the clouds. 
	Thus, it is important to keep in mind that the molecular clouds do not react immediately to SNe, and that the time between the explosion of a SN and the interaction of its shock front with one of the clouds varies depending on the distance between both sides, as well as as the composition of the ISM along the propagation front.
}

\textbf{
	\citetalias{IbanezMejia2017} make a beginning with discussing the influence of these SNe onto the modelled clouds.
	In particular, they focus on the overall properties of the clouds, like total mass, accretion rate, total velocity dispersion, and their evolution.
	They find that there is a tight correlation between those properties and that the way the clouds evolve strong depends on whether the clouds are hit by blast waves, as well as the details of these interactions (meaning, for example, the strength or direction of the shock jump).
...
\textbf{
	Note that these differences partly origin from the fact that the turbulence we study here is not isotropic, as it is assumed by \citet{She1994} and \citet{Boldyrev2002}. 
	This means that the derived VSFs are not only functions of time and value of lag scales, but also from the direction of $\vec{\ell}$ as that direction defines at which scales of $\ell$ large scale structures become relevant.
	...
	It also explains why comparisons with theoretical predictions are difficult although we are able to resolve the entire dissipation and inertial ranges \citepalias[their Appendix B]{IbanezMejia2017}.
}
...
\textbf{However, there are also} cases in which the 1D VSF evolves temporarily or completely differently than the 3D VSF for a short period of time.

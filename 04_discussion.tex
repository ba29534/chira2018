...
\jcim{Move to results: 
In Sect.~\ref{results:densthres} we have tested the influence of the density threshold that defines the volume of interest on the behaviour of the VSF.
...
\jcim{Removing the density threshold, by setting n$_\mathrm{cloud}$~=~0~cm$^{-3}$, would mean that we analyse the data of the entire data cubes.
...
\jcim{Furthermore, note that by choosing the starting points randomly we ensure that all parts of the cubes are considered. 

...
\abstract
	{ %context    	
%mm For a long time it has been argued that turbulence is an important
%requirement for the 
                Supersonic disordered flows accompany the 
        formation and evolution of molecular clouds (MCs). 
%mm It is supposed to be the dominant process that stabilises MCs
                 It has been argued that this is turbulence that can support
       against gravitational collapse and 
%mm supports
                 promote 
       the formation of hierarchical sub-structures. Yet, little is still known about 
%mm the sources of turbulence that dominates the 
                 what drives gas dynamics on MC scales.
%mm of entire MCs.
    	}
	{ %aims
...
%mm turbulence 
               gas dynamics
        within simulated MCs. We focus on the following questions:
        What physical process dominantes the driving of
%mm turbulence within MCs?  simulated MCs. We focus on the following
%questions: What \textbf{physical process dominates the driving of
%turbulent motions} within MCs? And is there a method that traces
%these dominant modes based in both simulated and observational data? 
                the disordered flows seen?  How can these flows be
                characterized? Are they consistent with turbulence? 
                Do the simulated flows agree with the observations?
    	} 
	{ %methods 
	We follow the gas motions within three MCs that have formed
        ...
        including self-gravity, magnetic fields, supernovae-driven
        turbulence, and radiative heating and cooling. 
%mm We express the distribution of turbulent power in terms of
                 We characterize the flow using 
     velocity structure functions n(VSFs) and compare the obtained parameters with predicted 
%mm
                and observed
values.
    	}
	{ %results
    	We demonstrate that the scaling exponent of VSFs and the
        self-similarity parameter are sensitive tools that trace the
        dominant driving sources of 
%mm turbulence.
               the flow. 
       The trends are generally robust against the influence of
       projection, Jeans refinement level, and density-weighting. Yet,
       the detailed evolution may vary significantly depending on the
       density threshold. 
    	}
	{ %conclusions
	...
	Gravitational contraction causes a flattening of the
          scaling exponent of the structure functions, but has no
          imprint on the self-similarity parameter of the structure
          function, unless the collapse is triggered by a
          shock. Nearby SN explosions inject turbulent energy on a
          wide range of scales onto  MCs. This injection of energy is
          visible in in the structure functions and the
          self-similarity parameters, but only for a short period of
          time as the excess kinetic energy decays.
 {\bf [comparison to observations?]}
    	}
